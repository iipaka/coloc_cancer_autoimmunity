# ==============================================================================
# Purpose: Generate Manhattan plots (GWAS <> eQTL) on Centered Data (PRE-COLOC)
# ==============================================================================
library(ggplot2)
library(dplyr)
library(data.table)
...
for(i in 1:nrow(sig_snps)) {
  ...
  for(eqtl_f in eqtl_files) {
    ...
    combined_plot <- (p1 / p2) + 
      plot_annotation(
        title = paste0("Manhattan Plot (centered around top SNP): ", clean_snp, " (Chromosome ", target_chr, ")"),
