# ==============================================================================
# Purpose: Dynamically generate regional Manhattan plots for target SNPs
# ==============================================================================
library(ggplot2)
library(dplyr)
library(data.table)
...
# 1. Create a folder to store the output images
dir.create("Output/Plots", showWarnings = FALSE, recursive = TRUE)

# 2. Load reference files 
sig_snps <- read.csv("Output/sig_snps_threshold_5e-08.csv", stringsAsFactors = FALSE)
gwas_info <- read.csv("Data_raw/gwas_summarystats.csv", stringsAsFactors = FALSE)

# ==============================================================================
# 3. DEFINE PLOTTING THEME & FUNCTION
# ==============================================================================
...
manhattan_plot_fx <- function(data, title, shared_pos, target_pos, target_name, dot_colour, x_min, x_max) {
  ...
  ggplot(data, aes(x = POS / 1e6, y = logP)) +
    ...
    annotate("text", x = Inf, y = gws_threshold, label = "Genome-wide significance", 
...
for(i in 1:nrow(sig_snps)) {
  ...
  target_pos  <- as.numeric(sig_snps[i, 3])
  ...
  # D. Safety check
  if(!file.exists(file_cancer) | !file.exists(file_immune)) {
    message("  -> WARNING: Missing .rds file(s). Skipping...")
    next 
  }
  
  # E. Load the data and format into dataframes
  ...
  # I. Clean filename and save
  safe_rsid <- gsub(":", "_", rsid)
  plot_filename <- paste0("Output/Plots/Plot_", safe_rsid, "_", cancer_name, "_vs_", immune_name, ".png")
