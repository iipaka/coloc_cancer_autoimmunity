...
for(i in 1:nrow(gwas_summarystats)) {
    ...
    header_cmd <- paste("zcat", f_path, "| head -n 1") #Read header of file
    header <- names(fread(cmd = header_cmd)) #Store header names
...
# ------------------------------------------------------------------------------
    ...
    #Extract only rows where it's the header or the chromosome matches + position within defined 1MB window
...
# ------------------------------------------------------------------------------
    ...
    #If beta is missing but OR exists, calculate beta = log(OR)
    ...
    #If SE is missing but CI exists, calculate SE from CI using 3.92 Z-score approximation for 95% CI
    ...
    #Create unique identifier combining chr and pos
