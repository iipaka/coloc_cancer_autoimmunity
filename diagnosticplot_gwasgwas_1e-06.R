...
out_dir <- "Output/Diagnostic_Plots_GWAS"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

gwas_info <- read.csv("Data_raw/gwas_summarystats.csv")
p_threshold <- "1e-06"
coloc_res <- read.csv(paste0("Output/coloc_results_summary_", p_threshold, ".csv"))
...
for(i in 1:nrow(lead_coloc_snps)) {
  ...
  rsid <- lead_coloc_snps$SNP[i]
  trait1_name <- lead_coloc_snps$Cancer_trait[i] 
  trait2_name <- lead_coloc_snps$Immune_trait[i]
